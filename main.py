...
START_VERIFY_DIALOG = "div.el-dialog__title:contains('Verify to start')"
...
class BytenutRenewal:
    ...
    def get_start_status(self, sb, server_id):
        referer = f"https://www.bytenut.com/free-gamepanel/{server_id}"
        return self.call_api(sb, API_START_STATUS.format(server_id), referer=referer)
    ...
    # ---------- 等待弹窗内 Turnstile / Continue 可用 ----------
    def _wait_dialog_turnstile(self, sb, timeout=30):
        deadline = time.time() + timeout
        while time.time() < deadline:
            dialog_gone = sb.execute_script("return !document.querySelector('div.el-dialog');")
            if dialog_gone:
                self.log("✅ 弹窗已自动消失")
                return True
            ...
            """)
            if enabled:
                self.log("✅ Continue 按钮已启用")
                return True
            time.sleep(1)
        self.log("⚠️ 弹窗 Turnstile 等待超时")
        return True
    ...
    def _handle_queue_dialog(self, sb):
        try:
            for _ in range(5):
                has_q = sb.execute_script("return !!document.querySelector('div.el-message-box.queue-dialog-styled');")
                if has_q:
                    break
                time.sleep(1)
            if has_q:
                self.log("📋 排队弹窗出现，点击 OK...")
                sb.execute_script("""
                    document.querySelectorAll(
                        'div.el-message-box.queue-dialog-styled .el-message-box__btns button'
    ...
    # ---------- 轮询开机状态 ----------
    def poll_start_status(self, sb, server_id, timeout=60, interval=5):
        deadline = time.time() + timeout
        while time.time() < deadline:
            ...
            if data:
                ...
                self.log(f"开机状态: inQueue={in_queue}, canStart={can_start}, msg={msg}")
                if not in_queue and can_start:
                    return True, "running"
            time.sleep(interval)
        return False, "start_timeout"

    # ---------- 轮询服务器运行状态（使用 server list API） ----------
    ...
    # ---------- 增强的 UI 开机流程 ----------
    def ui_start_server(self, sb, server_id):
        self.log("🔧 进入增强 UI 开机流程...")
        if f"/free-gamepanel/{server_id}" not in sb.get_current_url():
            sb.uc_open_with_reconnect(f"https://www.bytenut.com/free-gamepanel/{server_id}", reconnect_time=6)
            time.sleep(5)

        # 1. 点击 Management 菜单（JS）
        self.log("⚙️ 点击 Management 菜单...")
        try:
            sb.execute_script("""
                var items = document.querySelectorAll('.el-sub-menu__title span');
                for(var i=0; i<items.length; i++){
                    if(items[i].textContent.trim() === 'Management'){
                        items[i].closest('.el-sub-menu__title').click();
                        break;
                    }
                }
            """)
            ...
            self.log(f"Management 点击失败: {e}")
            return False, "management_menu_fail"

        # 2. 点击 Console 菜单（JS）
        self.log("🖥️ 点击 Console 菜单...")
        try:
            sb.execute_script("""
                var items = document.querySelectorAll('.el-menu-item span');
                for(var i=0; i<items.length; i++){
                    if(items[i].textContent.trim() === 'Console'){
                        items[i].closest('.el-menu-item').click();
                        break;
                    }
                }
            """)
            ...
            return False, "console_menu_fail"
        ...
        try:
            ...
            self.log("⚠️ 等待 Start 按钮超时")
            return False, "no_start_btn"

        # 4. 点击 Start
        self.log("▶️ 点击 Start 按钮...")
        self.remove_overlay_ads(sb)
        try:
        ...
        for idx, (user, pwd) in enumerate(accounts, 1):
            ...
            with SB(
                ...
                try:
                    ...
                    if state == "offline":
                        if can_extend:
                            ...
                            if result is True:
                                ...
                                # 增强的 UI 开机
                                success, msg = self.ui_start_server(sb, server_id)
                                if not success:
                                    self.send_tg("❌", "续期成功但开机失败", user, server_id,
                                                 "offline", new_time, msg,
                                                 screenshot=self.shot(sb, f"start_fail_{idx}.png"))
                                    continue

                                # 确认服务器运行
                                running, final_state = self.wait_until_running(sb, server_id)
                                final_shot = self.shot(sb, f"final_{idx}.png")
                                if running:
                                    ...
                                    self.send_tg("⚠️", "续期成功，开机已请求但未确认运行", user, server_id,
                            ...
                            if expired:
                                ...
                                success, msg = self.ui_start_server(sb, server_id)
                                if not success:
                                    self.send_tg("❌", "开机失败", user, server_id, "offline", expiry_str, msg,
                                                 screenshot=self.shot(sb, f"start_fail_{idx}.png"))
                                    continue
                                running, final_state = self.wait_until_running(sb, server_id)
                                final_shot = self.shot(sb, f"final_{idx}.png")
                    ...
                    time.sleep(3)
                    result, new_time = self.try_extend_and_verify(sb, server_id, expired_time)
                    final_shot = self.shot(sb, f"final_{idx}.png") if result is True else None
                    if result is True:
